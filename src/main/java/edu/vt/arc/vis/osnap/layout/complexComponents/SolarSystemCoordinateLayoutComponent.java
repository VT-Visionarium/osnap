...
import edu.vt.arc.vis.osnap.graph.algorithms.SimpleDistanceWeight;
import edu.vt.arc.vis.osnap.graph.algorithms.minimumSpanningTree.PrimsAlgorithm;
...
/**
 * The {@link SolarSystemCoordinateLayoutComponent} class provides a coordinate
 * layout in the shape of a solar system. The layout calculates a minimum
 * spanning tree from a provided root node. This can be based on a provided
 * {@link Metadata} criterion that filters the edges included to those with the
 * criterion. It lays out all nodes radially below their parent nodes in the
 * spanning tree with the root node in the origin. Provided parameters can
 * invert the direction of the spanning tree, specify the maximum size and
 * minimum distance of the nodes to prevent overlap, as well as scale the depth
 * of the tree.
 * 
...
public class SolarSystemCoordinateLayoutComponent
    ...
    /**
     * Returns the root {@link INode Node}.
     * 
     * @return the root {@link INode Node}.
     */
     ...
     * edges or <code>null</code>, if the LayoutComponent is unconstrained.
     * 
     ...
     *            the maximum radius.
     */
    public void setMaximumNodeSize(float maximumNodeRadius) {
     ...
     * {@link SolarSystemCoordinateLayoutComponent} class. (Serialization
     * Constructor).
     */
    ...
    /**
     * Creates a new instance of the
     * {@link SolarSystemCoordinateLayoutComponent} class with the provided
     * root {@link INode Node}.
     * 
     * @param rootNode
     *            the root {@link INode Node}.
     */
    public SolarSystemCoordinateLayoutComponent(INode rootNode) {
    ...
    /**
     * Creates a new {@link SolarSystemCoordinateLayoutComponent} with the
     * provided root {@link INode Node}, limiting paths from the root node to
     * edges that contain the provided {@link Metadata}. Furthermore, the
     * minimal distance between nodes and the maximum distance of nodes is set
     * to 1. The depth of the resulting coordinate layout is not scaled.
     * Finally, the path from the root node to each individual node is inverted
     ...
     * edges that contain the provided {@link Metadata}. Furthermore, the
     * minimal distance between nodes and the maximum distance of nodes is set
     * to the provided values. Furthermore, the depth of the resulting
     * coordinate layout is scaled by the depth modifier.
     * 
     * @param rootNode
     *            the root {@link INode Node}.
     * @param metadata
     ...
     */
    public SolarSystemCoordinateLayoutComponent(INode rootNode,
            Metadata metadata, float minimalDistance, float maximumNodeRadius,
            float depthModifier) {
     ...
     * provided root {@link INode Node}, limiting paths from the root node to
     * edges that contain the provided {@link Metadata}. Furthermore, the
     * minimal distance between nodes and the maximum radius of nodes is set to
     * the provided values. Furthermore, the depth of the resulting coordinate
     * layout is scaled by the depth modifier. Finally, the path from the root
     * node to each individual node is inverted depending on the provided
     * parameter.
     * 
     * @param rootNode
     *            the root {@link INode Node}.
    ...
    public SolarSystemCoordinateLayoutComponent(INode rootNode,
        ...
        super(SolarSystemCoordinateLayoutComponent.name(),
                SolarSystemCoordinateLayoutComponent.description(), true,
                serialization);
    ...
    private Number calculateDepth(INode node) {
    ...
    protected class ComponentNode {
        ...
        private void calculateRadius() {
            ...
            float circumference = (float) (2f * Math.PI * SolarSystemCoordinateLayoutComponent.this.maximumNodeRadius);
            ...
            // the children.
            float childDiameters = SolarSystemCoordinateLayoutComponent.this.minimalDistance
                    * this.children.size();
        ...
        private float calculateAngleForChild(ComponentNode child,
            ...
            float distance = childRadius + grandChildRadius + predecessorRadius
                    + predecessorGrandChildRadius + minimalDistance;
        ...
        public void setChildPositions() {
            ...
            for (ComponentNode child : this.getChildren()) {
                ...
                child.setThirdComponent(calculateDepth(child.getNode()));
        ...
        public void setFirstComponent(Number firstComponent) {
            ...
            this.position = new Point4<>(firstComponent,
                    this.position.getY(), this.position.getZ(), Float.class);
        }
        ...
        public void setSecondComponent(Number secondComponent) {
            ...
            this.position = new Point4<>(this.position.getX(),
                    secondComponent, this.position.getZ(), Float.class);
        }
